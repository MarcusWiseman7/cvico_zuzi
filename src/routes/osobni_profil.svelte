...
{#if profile?.pic}
    <ZImage src={profile.pic} alt="Zuzana" zclass="w-full rounded-lg shadow-lg" />
{/if}

{#if profile?.items?.length}
    <section class="mt-10">
        {#each profile.items as info}
            <InfoBlock {info} />
        {/each}
    </section>
{/if}

{#if profile?.bottomPics?.length}
    <section class="grid gap-5 grid-cols-3">
        {#each profile.bottomPics as pic}
            <ZImage src={pic} alt="yoga pic" zclass="rounded-lg shadow-lg" />
        {/each}
    </section>
{/if}
